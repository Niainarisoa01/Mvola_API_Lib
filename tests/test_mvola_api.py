...
from mvola_api.constants import SANDBOX_URL
...
class TestMVolaAuth(unittest.TestCase):
    ...
    @patch('mvola_api.http_client.SecureHTTPClient.post')
    ...
    def test_generate_token_failure(self, mock_post):
        ...
        with self.assertRaises(MVolaAuthError):
            self.auth.generate_token(force_refresh=True)
    ...
    def test_validate_transaction_params_invalid_description(self):
        ...
        with self.assertRaises(MVolaError):
            self.transaction._validate_transaction_params(
                ...
                debit_msisdn="0340000001",
                credit_msisdn="0340000002",
                description="Test transaction with special characters: #$%^"  # Invalid
            )
...
class TestMVolaClient(unittest.TestCase):
    ...
    def test_sandbox_property(self):
        """Test that sandbox is accessible as a property."""
        self.assertTrue(self.client.sandbox)

    def test_no_public_credentials(self):
        """Test that credentials are NOT accessible publicly."""
        self.assertFalse(hasattr(self.client, "consumer_key"))
        self.assertFalse(hasattr(self.client, "consumer_secret"))
